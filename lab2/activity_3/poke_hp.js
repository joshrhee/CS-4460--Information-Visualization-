...
// filter "pokemonList" array based on "type". Then sort by "hp".
function updateBars(type) {
    ...
    const filteredPokemonList = pokemonList.filter((pokemon, _) => {
        return pokemon.type === type;
    })

    filteredPokemonList.sort((pokemonA, pokemonB) => {
        return pokemonA.hp - pokemonB.hp;
    })

    renderBars(filteredPokemonList);
}
