...
d3.csv("cereals.csv", function (csv) {
  ...
  function highlightBrushedCircles() {
      ...
      if(e) {
          ...
          selected = circles.filter((circle) => {
            let circleX = 0;
            let circleY = 0;
  ...
  function displayValues() {
    ...
    if (selected._groups[0].length !== 1) {
      clearSelectedCircleInfo();
      return
    } 

    console.log("selected length is 1!!!!!")
    const d = selected._groups[0][0].__data__;
    clearSelectedCircleInfo();

    d3.select("#Cereal")
      .text(d.CerealName)
      .attr("class", "Cereal-text");
    ...
    d3.select("#FatValue")
      .text(d.Fat)
      .attr("class", "FatValue-text");

    d3.select("#CrabValue")
      .text(d.Carb)
      .attr("class", "CrabValue-text");

    d3.select("#FiberValue")
      .text(d.Fiber)
      .attr("class", "FiberValue-text");

    d3.select("#ProteinValue")
      .text(d.Protein)
      .attr("class", "ProteinValue-text");
  }

  function clearSelectedCircleInfo() {
    d3.select(".Cereal-text")
      .text("")

    d3.select(".Calories-text")
      .text("")

    d3.select(".FatValue-text")
      .text("")

    d3.select(".CrabValue-text")
      .text("")

    d3.select(".FiberValue-text")
      .text("")

    d3.select(".ProteinValue-text")
      .text("")
  }
